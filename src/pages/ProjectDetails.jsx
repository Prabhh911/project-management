...
export default function ProjectDetail() {
    ...
    // FIX: also read loading so we don't show "not found" while data is still fetching
    ...
    useEffect(() => {
        if (!id) return;
        // FIX: removed `projects.length > 0` guard — search regardless of array length
        // Also compare both as strings to handle numeric IDs from DB
    ...
    // FIX: show spinner while workspace is loading instead of "Project not found"
    if (loading) return (
    ...
    // FIX: null-safe status for newly created projects that have no status set
            ...
            <div className="flex max-md:flex-col gap-4 flex-wrap items-start justify-between max-w-6xl">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <span className={`px-2 py-1 rounded text-xs capitalize ${statusColors[status]}`}>
                            {status.replace("_", " ")}
                        </span>
                    </div>
                </div>
