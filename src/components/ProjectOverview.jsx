...
const ProjectOverview = () => {
    ...
    // FIX: safe date format helper - never crashes on null
                ...
                {projects.length === 0 ? (
                    ...
                    <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                        {projects.slice(0, 5).map((project) => {
                            // FIX: null-safe status and priority
                                ...
                                >
                                    <div className="flex items-start justify-between mb-3">
                                        ...
                                        <div className="flex items-center gap-2 ml-4">
                                            <span className={`text-xs px-2 py-1 rounded ${statusColors[status] || statusColors["PLANNING"]}`}>
                                                {status.replace(/_/g, " ")}
                                            </span>
                                            <div className={`w-2 h-2 rounded-full border-2 ${priorityColors[priority] || priorityColors["MEDIUM"]}`} />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-4 text-xs text-zinc-500 dark:text-zinc-500 mb-3">
                                        ...
                                        {/* FIX: was format(new Date(null)) which throws */}
                                        {safeFormat(project.end_date, "MMM d, yyyy") && (
                                            <div className="flex items-center gap-1">
